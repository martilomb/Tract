...
- `connector_test_runs` records the tenant-scoped configuration version, test mode, sanitized result/status, timing, actor, and audit evidence. It never stores a credential value or provider response body. Live-mode rows are rejected unless the connector is approved and carries the required opaque specification/secret references; configuration and synthetic modes remain safe credential-free validation.
...
Domain rules live in `src/domain/ingestion.ts`, file staging in `src/domain/imports.ts`, provider contracts in `src/domain/connectors.ts`, the runtime adapter in `src/server/connector-runtime.server.ts`, and the canonical schema in migrations `202608210003` and `202608240001`–`003`.
