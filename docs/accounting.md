...
- The production Contracts journey persists those same prerequisites as exact tenant-scoped terms. The current approved no-provider path is an administrator's manual attestation to an executed agreement held in the controlled external register; it records a reference, summary, reviewer, and time without pretending a scanner or extractor ran. Activation creates the agreement-bound accrual and approved rate records atomically, but it does not create a volume event, recovery calculation, general-ledger entry, claim, or settlement posting.
- The synthetic Contracts journey remains available only in visibly labelled demo mode. It applies the same prerequisites before returning a local Active agreement, produces no accounting posting, and cannot satisfy production acceptance.

The TypeScript calculation is in `src/domain/recovery.ts`; source qualification is in `src/domain/ingestion.ts`. Database storage and constraints are in migrations `202608210002_product.sql`, `202608210003_ingestion_domains.sql`, the additive Milestone 10 migrations through `202608240003_milestone10_acceptance_controls.sql`, the production Contracts migrations `202608290001`–`002`, and the P3 governed master-data/effective-revision migrations `202608290003`–`005`. P3 exposes only persisted approved agreement/accrual totals by currency and withholds volume, recovery, forecast, variance, and break-even analytics until P4/P5 produce approved calculation records.
