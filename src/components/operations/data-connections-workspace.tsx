...
export function DataConnectionsWorkspace({
  ...
  onOpenImports?: () => void;
  onValidatedDraft?: () => void;
}) {
  const navigate = useNavigate();
  const openImports = onOpenImports ?? (() => void navigate({ to: "/operations" }));
  ...
  const wizardTriggerRef = useRef<HTMLButtonElement | null>(null);
  const selected =
  ...
  const openWizard = (step = 0, trigger?: HTMLButtonElement) => {
    if (trigger) wizardTriggerRef.current = trigger;
    setWizardStart(step);
    setWizardOpen(true);
  };
  const changeWizardOpen = (open: boolean) => {
    setWizardOpen(open);
    if (!open) requestAnimationFrame(() => wizardTriggerRef.current?.focus());
  };
  return (
    <>
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <Action
          ...
          onClick={(trigger) => openWizard(0, trigger)}
        />
        <Action
        ...
        <Action
          ...
          onClick={(trigger) => openWizard(3, trigger)}
        />
        <Action
          icon={FlaskConical}
          label="Test connection"
          detail="Fail-closed server boundary"
          onClick={(trigger) => openWizard(4, trigger)}
        />
      </div>
      ...
      <ConnectionWizard
        ...
        onValidatedDraft={onValidatedDraft}
      />
    </>
  );
}
