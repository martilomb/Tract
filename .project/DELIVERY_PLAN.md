...
| Master-data boundaries                  | P2 governed creation passed; full P3 workspace/review UX open          | Vehicle programs, part numbers/revisions, DCRs, and recovery agreements are distinct records and workflows. P2 transactionally creates a compatible canonical set with case-insensitive duplicate denial and immutable proposal evidence; broader search/alias/merge/effective-date analysis remains P3.    |
...
| Enterprise UX                           | P1–P2 production slices passed; P3–P10 remain gated                    | The non-demo session/organization and Contracts workflows are real. Other product routes remain fixture- or browser-state-backed or unavailable; each still requires authenticated Supabase actions, reload persistence, authorized retrieval, audit, and reconciled projections/exports before acceptance. |
...
3. **Programs and Parts — active:** production-backed master-data search, duplicate/alias/provenance review, effective-dated revisions, scoped detail and analytical projections, evidence, export, and refresh persistence.
4. **Ingestion and time-aware ledger:** real immutable raw/staged/candidate/posting lifecycle, approved mappings, corrections/reversals/returns, qualified eligible volume, exact effective-time rate selection, idempotent ledger posting, reconciliation, retry/cancel, and audit. Live providers remain disabled until activated.
