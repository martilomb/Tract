...
| Enterprise UX                           | Bounded demo increments verified; production routes not accepted       | The non-demo session/organization spine is real; product workflow routes remain fixture- or browser-state-backed. Production acceptance requires authenticated Supabase repositories/actions, reload persistence, authorized retrieval, audit, and reconciled projections/exports for every enabled workflow.              |
...
1. **Application spine:** authenticated tenant/session context; production Supabase repositories, loaders, and server actions; invitation and membership lifecycle; administrator/full-view/program/part/department/team scopes enforced by database and server; provider-neutral plan, subscription, seat-entitlement, and audit model; server-side seat enforcement; real empty/loading/error states. Billing-provider IDs remain opaque and optional. Stripe test-mode checkout, portal, and idempotent verified webhooks may be added only in the later billing slice and never with live charging or source-stored secrets.
2. **Contracts and atomic recovery activation:** complete the golden production workflow. An invited organization administrator signs in; selects or creates governed canonical program/model-year and part/revision records; creates or uploads and reviews an agreement; enters approved terms, rates, eligible-volume basis, currency, rounding, dates, and forecast assumptions; activates atomically; reloads; and retrieves identical records and audit history. A failed step retains an incomplete draft and no partial active state.
3. **Programs and Parts:** production-backed master-data search, duplicate/alias/provenance review, effective-dated revisions, scoped detail and analytical projections, evidence, export, and refresh persistence.
