...
| DCR workflow                            | Milestone 10 acceptance reopened; corrective work approved           | Table and configurable board views, guarded transitions, controlled affected-record selection, typed governed properties, evidence, assignments, comments, approvals, activity history, and an agreement/setup gate at canonical Active without requiring an agreement to draft. |
| Contracts and recovery agreements       | Milestone 10 acceptance reopened; atomic activation work approved    | A separate Contracts area and one guided recovery setup that joins an optional approved DCR, approved agreement evidence, controlled vehicle/part records, effective rates, volume basis, assumptions, and transactional activation without partial active recovery.             |
| Master-data boundaries                  | Distinct schema and creation workflows implemented and verified      | Vehicle programs, part numbers/revisions, DCRs, and recovery agreements are distinct records and workflows. A redesigned part creates a DCR and effective-dated part revision; a new program is created only for a genuinely new vehicle program/carline.                        |
...
| Enterprise UX                           | Demonstrated defects; Milestone 10 acceptance reopened               | Plain-language, permission-aware workflows with bounded accessible overlays, one coherent URL-preserved analysis scope, bounded data views, real synthetic actions/exports, and no mixed-scope, no-op, or mock/live claims.                                                      |
| Forecasting                             | Calculation core retained; scoped experience and reconciliation open | Versioned forecast projections must use the canonical aggregation layer and support program and part scope, chart/table views, scoped exports, provenance, and reconciliation to the same source records. Unsupported predictive claims remain excluded.                         |
...
10. **Product model and enterprise UX correction:** complete the reopened A–G correction above: canonical scoped analysis, bounded accessible overlays, atomic recovery setup, configurable DCR pipelines/properties/materiality, controlled master data, actionable contracts/recoveries/reports, unified Operations, honest Settings, and a new product-owner acceptance pass.
