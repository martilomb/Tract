...
| --------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Recovery accounting                     | Schema/domain verified; production application workflow pending        | Recoverable cost, rate periods/effective dates, precise decimals, contract/accrual-specific eligible-volume basis, recovered/remaining/percent/completion/variance, provenance, revisions, reproducible rules, drill-down, exceptions, reports, filters, exports, audit history.                                             |
| DCR workflow                            | Schema/domain and demo UX verified; production persistence pending     | Table and Board views, guarded drag/drop and accessible stage control, controlled affected-record selection, evidence, assignments/reviewers, private attachments, comments, approvals, and activity history, with an agreement/setup gate at Active without requiring an agreement to draft.                                |
| Contracts and recovery agreements       | P2 production-backed staging acceptance passed                         | The non-demo Contracts area persists tenant-scoped governed master data, incomplete agreement drafts, reviewed manual evidence, exact terms/rates, canonical links, atomic activation, active accrual provenance, audit, and reconciled scoped CSV; live document providers remain disabled.                                 |
| Master-data boundaries                  | P3 production-backed staging acceptance passed                         | Vehicle programs, part numbers/revisions, DCRs, and recovery agreements remain distinct. Non-demo Programs and Part Numbers reuse the P2 canonical records through tenant-scoped search/detail/export, administrator-only governed creation, duplicate denial, effective revisions, approved aliases, provenance, and audit. |
...
| Shared ingestion controls               | Staging schema/domain passed; production actions and retrieval pending | Immutable source objects/raw rows separated from normalized candidates; Received→Staged→Validated→Mapped→Reviewed→Approved→Posted; rejected/failed traceability; duplicate economic-event prevention; no executable mappings.                                                                                                |
| Identity and authorization              | P1 production-backed staging acceptance passed                         | Authenticated tenant/session resolution, invitations, multi-organization membership, administrator/full-view/scoped enforcement, seat entitlements, audit, reload persistence, MFA, and revocation pass against staging; customer SSO and paid session controls remain activation items.                                     |
| Enterprise UX                           | P1–P3 production slices passed; P4–P10 remain gated                    | The non-demo session/organization, Contracts, Programs, and Part Numbers workflows use real staging persistence. Other product routes remain fixture- or browser-state-backed or unavailable until their authenticated Supabase actions, reload, authorization, audit, and reconciliation gates pass.                        |
| Forecasting                             | Demo analytical contract verified; persisted forecasting pending       | Versioned forecast projections must use persisted tenant data and progress to explainable baseline/seasonal/trend candidates, rolling-origin backtesting, per-series selection, hierarchy reconciliation, confidence ranges, scenario overrides, and provenance.                                                             |
...
## Mandatory production-completion strategy — 27 August 2026

This strategy supersedes synthetic route coverage, isolated schema tests, and visual polish as completion gates. Existing work remains valuable foundation and explicit demonstration behavior, but it is not production functionality until the real application uses it through authenticated, tenant-scoped Supabase persistence.

### Definition of a completed workflow

A workflow is complete only when an authorized non-demo user can perform it through the real application, reload, retrieve the persisted result, and see the same tenant-scoped data reconcile across detail, chart, table, export, provenance, and audit history. Every enabled control must perform its stated action; otherwise it is absent or disabled with the exact credential, policy, provider, or customer decision needed. Browser-local state and fixtures remain permitted only behind the explicit demonstration boundary and never satisfy production acceptance.

Every production slice must deliver together:

- additive database changes where required, RLS/tenant-denial pgTAP, domain invariants, typed repositories, server loaders/actions, and generated database types;
- frontend integration tests and real-browser E2E in non-demo mode, including reload persistence, authorized and denied roles, loading/empty/error states, audit evidence, desktop and 390 px mobile, keyboard/accessibility sampling, and zero unexplained console/network errors;
- one canonical data/query/calculation layer with invariant tests proving contract → program/model year → part/revision → rate/eligible volume → ledger result and chart ↔ table ↔ detail ↔ export reconciliation;
- explicit residual risk, provider/credential boundary, cost impact, and staging evidence before publication.

### Dependency-ordered vertical slices

1. **Application spine — accepted P1:** authenticated tenant/session context; production Supabase repositories, loaders, and server actions; invitation and membership lifecycle; administrator/full-view/program/part/department/team scopes enforced by database and server; provider-neutral plan, subscription, seat-entitlement, and audit model; server-side seat enforcement; real empty/loading/error states. Billing-provider IDs remain opaque and optional. Stripe test-mode checkout, portal, and idempotent verified webhooks may be added only in the later billing slice and never with live charging or source-stored secrets.
2. **Contracts and atomic recovery activation — accepted P2:** the golden production workflow persists governed master records and incomplete drafts, records reviewed manual evidence and exact terms/rates, activates through one transaction, reloads identical authorized data/audit/provenance, and leaves no partial active state on failure. Live upload/scanning remains disabled pending its named activation inputs.
3. **Programs and Parts — accepted P3:** production-backed master-data search, duplicate/alias/provenance review, effective-dated revisions, scoped persisted detail/evidence/export, role and tenant enforcement, audit, and refresh persistence pass. Financial analytics remain absent until P4/P5 supply approved ledger and forecast records rather than synthesized values.
4. **Ingestion and time-aware ledger — active:** real immutable raw/staged/candidate/posting lifecycle, approved mappings, corrections/reversals/returns, qualified eligible volume, exact effective-time rate selection, idempotent ledger posting, reconciliation, retry/cancel, and audit. Live providers remain disabled until activated.
5. **Forecasting:** persisted explainable forecast versions with baseline, seasonal, and trend candidates; rolling-origin backtesting; per-series selection; hierarchy reconciliation; confidence ranges; governed scenario overrides; provenance; and exact ledger/report integration. No arbitrary weighted-trend production claim.
6. **Recoveries:** production tenant-scoped current positions, evidence-neutral under/over review, calculation replay, contract/part/program drill-down, materiality policies, and reconciled exports.
7. **Reports:** production filter → preview → generate/download for recovery, actual/contract/forecast, variance, DCR aging, ingestion reconciliation, and audit/evidence outputs; current scope, permissions, provenance, and format parity are mandatory.
8. **Operations, Settings, and billing:** persisted connectors/import monitoring, document review, exceptions, rules, organization/personal administration, invitations/deprovisioning, seat usage/enforcement, security activation boundaries, and optional provider-neutral billing adapter. Stripe remains test-mode only until separately approved.
9. **DCR:** production Table/Board workflow, fixed guarded lifecycle, controlled associations, evidence/private attachments, comments, assignments/reviewers, approvals, activity, reload persistence, and agreement/setup gate at Active.
10. **Overview and release hardening:** only after upstream slices, compose reconciled production aggregates and alerts; complete the route/control, database/Auth/Storage, browser/accessibility, dependency/security, staging, operations, and product-owner evidence matrix.

User testing begins only after automated slice gates pass. It evaluates workflow fitness, terminology, usefulness, and visual quality; it is not the mechanism for finding persistence, authorization, reconciliation, accessibility, or runtime defects that the defined automation can catch.
