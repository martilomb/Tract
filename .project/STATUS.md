...
All useful Milestone 1–9 production work is complete. The approved GitHub repository is connected and the existing `main` history is published without rewriting it. Automatic GitHub Actions execution is disabled because the account's hosted Actions credits are exhausted; the verified workflow remains available for manual execution after credits reset. The project-scoped Supabase MCP server is loaded and OAuth-authenticated. Read-only preflight inspection confirmed that it is bound to the documented non-production staging reference `qflwjgmrspmcyghzinwz`, with no application migrations, public tables, Auth users, Storage buckets, Storage objects, or application policies; pre-migration security and performance advisors are clean. Hosted activation remains fail-closed, modular, and approval-gated.
...
- Exact-version staging migration application requires an approved Supabase CLI authentication/link method. The current MCP `apply_migration` operation generates a new server-side version instead of preserving the repository's existing `202608210001`–`003` versions; using it would create migration-ledger drift. Do not apply these existing files through MCP or edit/repair the ledger manually. Once CLI access is supplied through the approved credential path, use the pinned CLI to push the unchanged files and verify that the remote ledger versions, names, order, and schema match before generating types or running the hosted suites.
...
Provide the approved Supabase CLI authentication/link method for non-production project `qflwjgmrspmcyghzinwz`, then use pinned CLI `2.111.0` to push only unchanged migrations `202608210001`–`003` and prove the remote ledger matches their repository versions. Run hosted database validation, generate and wire project database types, and execute the available RLS/Auth/Storage/ingestion checks before staging deployment. Activate IHS/AFS, SAP, scanning/extraction, email, SSO, or other providers independently only after the corresponding specifications, samples, licensing/security approval, credentials, and reconciliation evidence in [docs/external-activation.md](../docs/external-activation.md) are complete.
