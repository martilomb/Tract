...
- `pnpm check`: passed (format, zero-warning lint, TypeScript, 17 tests across 6 files, and production build).
...
- Supabase pgTAP/RLS tests are authored and wired into CI but cannot be executed until a local container runtime or approved Supabase project is available.
...
- Production volume, document extraction/scanning, notification, and SSO providers require approved specifications, credentials, data-processing terms, and any paid-service approval.
...
After the product owner provides an approved Supabase project or local container runtime, apply both migrations to a fresh database, run `supabase db lint` and `supabase test db`, generate database types, then execute authenticated anonymous/cross-tenant browser tests. In parallel, a Git remote can be connected only after explicit approval.
