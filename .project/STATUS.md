...
All useful Milestone 1–9 implementation work is complete. The approved GitHub repository is connected and the existing `main` history is published without rewriting it. Automatic GitHub Actions execution is disabled because the account's hosted Actions credits are exhausted; the verified workflow remains available for manual execution after credits reset. Supabase CLI `2.111.0` is authenticated and linked to the confirmed non-production staging project `qflwjgmrspmcyghzinwz`. The unchanged repository migrations `202608210001`–`003` are applied with an exact matching remote ledger, and the linked schema, database lint, RLS/ingestion pgTAP suites, Auth service boundary, and private Storage boundary have been verified. Hosted Auth workflow configuration, document scanning/signed-read behavior, restore evidence, Cloudflare deployment, and all paid or external providers remain fail-closed and approval-gated.
...
Resolve the hosted Auth configuration mismatch only after the approved staging site/redirect URLs, invitation/reset behavior, MFA/session policy, and test identities are supplied; then run the outstanding Auth and multi-organization browser acceptance. Independently review the recorded performance-advisor findings against expected workloads and the intentional additive authorization model before proposing a traceable hardening migration. Document upload/scan/signed-read, isolated restore, Cloudflare staging deployment, and live IHS/AFS, SAP, extraction, notification, or SSO activation remain blocked on the corresponding approvals and inputs in [docs/external-activation.md](../docs/external-activation.md).
